...
*Portfolio case study for coopernorman.dev. Public-safe: all performance figures are backtest/validation; no realized P&L; no secrets. Diagrams referenced below are to be produced for the site.*
...
*Figures embedded above are in `figures/`. The reliability curve is **illustrative** — regenerate it from your real model artifacts before publishing. Still to produce: a multi-leg copula-pricing flow diagram, and a 90-second walkthrough video.*
